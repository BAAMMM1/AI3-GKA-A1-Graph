...
public class GraphGenerator {
	 ...
	 */
	public Graph generateRandomSimpleGraph(int nodeSize, int edgeSize, int maxWeight) {
	...
	private void initGenerator(int nodeSize, int edgeSize, int maxWeight) {
		...
		this.graphResult = new MultiGraph("Random");
	...
	private void generateConnectedNodes() {
		...
		for (int i = 0; i < this.nodeSize - 1; i++) {
			...
			this.addEdge(graphResult, nodeRandomFromList, nodeNew);
			this.edgeSize--;
	...
	private void addSimpleEdgesToNodes() {
		...
		for (int i = 0; i < this.edgeSize; i++) {
			...
			if ((graphResult.getEdge((edgeID01)) == null) && (graphResult.getEdge((edgeID02)) == null)) {
				this.addEdge(graphResult, node01, node02);
			} else {
				i--;
				continue;
			}
	...
	private Node addNode(String name) {
		graphResult.addNode(name);

		Node node = graphResult.getNode(name);
